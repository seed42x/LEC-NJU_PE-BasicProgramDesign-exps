...
void Game::AIGame(int humanColor)
{
	...
	if (humanColor == 1)
	{
		...
		while (1)
		{
			pair<pair<int, int>, pair<int, int>> AIStep = reversi->step();
			...
			if (isGameOver() != -1) break;
			reversi->handleMessage(humanStep1.first, humanStep1.second, humanStep2.first, humanStep2.second, 1);
		}
	}
	else if (humanColor == 0)
	{
		...
		while (1)
		{
			...
			if (isGameOver() != -1) break;
			////////////////
			reversi->handleMessage(humanStep1.first, humanStep1.second, humanStep2.first, humanStep2.second,0);
			pair<pair<int, int>, pair<int, int>> AIStep = reversi->step();
			AIStep1 = AIStep.first;
			AIStep2 = AIStep.second;
			while (board[AIStep1.first][AIStep1.second] != -1)
